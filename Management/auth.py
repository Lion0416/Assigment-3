...
def login(): #view function
     ...
     if(login_form.validate_on_submit()==True):
        ...
        if error is None:
             login_user(u1)
             nextp = request.referrer #this gives the url from where the login page was accessed
             print(nextp)
...
def register():
    ...
    if (register.validate_on_submit() == True):
            ...
            address = register.residence_address.data
            ...
            new_user = User(userName=uname, password_hash=pwd_hash, emailid=email, contactNumber=contact, address=address )
